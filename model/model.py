...
def add_attn(node, field, attn):
    ...
    return {
        field:
        (attn(feat, node.mailbox['m'], node.mailbox['m']) + feat).squeeze(1)
    }
...
class Node_GRU(nn.Module):
    ...
    def forward(self, bg, suffix='h'):
        ...
        mask = (a_pharmj != 0).type(torch.float32).matmul(
            (p_pharmj.transpose(-1, -2) != 0).type(torch.float32)) == 0
        ...
        for i in range(bg.batch_size):
            start, size = start_index[i], node_size[i]
            graph_embed.append(h[i, :size].view(
                -1, self.direction * self.hid_dim).mean(0).unsqueeze(0))
...
def apply_custom_copy_src(g, etype, src_field, out_field, reduce_func):
    g.send_and_recv(g.edges(etype=etype),
                    message_func=partial(copy_src,
                                         src_field=src_field,
                                         out_field=out_field),
...
class MVMP(nn.Module):
    ...
    def __init__(self,
        ...
        if 'j' in view:
            ...
            self.hetero_etypes = [('a', 'j', 'p'),
                                  ('p', 'j', 'a')]  # don't have feature
    ...
    def update_node(self, node, field, layer):
        return {
            ...
            layer(
                torch.cat([
                    node.mailbox['mail'].sum(dim=1), node.data[field],
                    node.data['f']
                ], 1))
        }
    ...
    def forward(self, bg):
        suffix = self.suffix
        if not bg.edges[('p', 'r', 'p')].data['x'].numel() and len(
                self.homo_etypes) > 1:
        ...
        if 'j' in self.view:
            bg.nodes['a'].data[f'f_junc_{suffix}'] = bg.nodes['a'].data[
                'f_junc'].clone()
            bg.nodes['p'].data[f'f_junc_{suffix}'] = bg.nodes['p'].data[
                'f_junc'].clone()

        update_funcs = {
            e: (fn.copy_e('h', 'm'),
                partial(self.msg_func,
        ...
        for i in range(self.depth - 1):
            ...
            for edge_type in self.homo_etypes:
                ...
                bg.apply_edges(partial(del_reverse_message,
                                       field=f'f_{suffix}'),
                               etype=edge_type)
                bg.apply_edges(partial(
                    self.update_edge,
                    layer=self.mp_list[''.join(edge_type)][i]),
                               etype=edge_type)

        # last update of node feature
        update_funcs = {
            e: (fn.copy_e('h', 'mail'),
                partial(self.update_node,
                        field=f'f_{suffix}',
...
class PharmHGT(nn.Module):
    ...
    def __init__(self,
        ...
        self.mp = MVMP(msg_func=add_attn,
                       hid_dim=hid_dim,
                       depth=self.depth,
                       view='apj',
                       suffix='h',
        ...
        ## predict
        self.out = nn.Sequential(nn.Linear(4 * hid_dim, hid_dim), self.act,
                                 nn.Linear(hid_dim, hid_dim), self.act,
    ...
    def init_feature(self, bg):
        ...
        bg.nodes['a'].data['f'] = self.act(self.w_atom(
            bg.nodes['a'].data['f']))
        bg.edges[('a', 'b', 'a')].data['x'] = self.act(
            self.w_bond(bg.edges[('a', 'b', 'a')].data['x']))
        bg.nodes['p'].data['f'] = self.act(
            self.w_pharm(bg.nodes['p'].data['f']))
    ...
    def forward(self, bg):
        ...
        embed_a = torch.mean(torch.stack([embed_f_a, embed_junc_h_a], dim=1),
                             dim=1)
        embed_p = torch.mean(torch.stack([embed_f_p, embed_junc_h_p], dim=1),
                             dim=1)
