...
class Masking_Trainer(object):
    ...
    def __init__(self, cfg, global_rank, world_size, model_list, dataloaders,
                 criterion, optimizer_list, device, output_dir):
        self.cfg = cfg
    ...
    def train_epoch(self):
        ...
        for step, batch in enumerate(self.dataloaders['train']):
            ...
            ## loss for atoms
            pred_atom = self.linear_pred_atoms(
                atom_rep[batch.nodes['a'].data['mask'] == True])
            # print(pred_atom.double().shape)
            # print(batch.nodes['a'].data['label'][batch.nodes['a'].data['mask']==True][:,0].shape)
            loss = self.criterion(
                pred_atom.double(), batch.nodes['a'].data['label'][
                    batch.nodes['a'].data['mask'] == True][:, 0])
            acc_atom = compute_accuracy(
                pred_atom, batch.nodes['a'].data['label'][
                    batch.nodes['a'].data['mask'] == True][:, 0])
            ...
            if self.cfg.train.mask_pharm:
                pred_pharm = self.linear_pred_pharms(
                    pharm_rep[batch.nodes['p'].data['mask'] == True])
                loss += self.criterion(
                    pred_pharm.double(), batch.nodes['p'].data['label'][
                        batch.nodes['p'].data['mask'] == True])
                acc_pharm = compute_accuracy(
                    pred_pharm, batch.nodes['p'].data['label'][
                        batch.nodes['p'].data['mask'] == True])
                acc_pharm_accum += acc_pharm

            if self.cfg.train.mask_edge:
                masked_edge_index = batch.edge_index[:, batch.
                                                     connected_edge_indices]
                edge_rep = atom_rep[masked_edge_index[0]] + atom_rep[
                    masked_edge_index[1]]
                pred_edge = self.linear_pred_bonds(edge_rep)
                loss += self.criterion(pred_edge.double(),
                                       batch.mask_edge_label[:, 0])

                acc_edge = compute_accuracy(pred_edge,
                                            batch.mask_edge_label[:, 0])
            ...
            if (self.global_train_step +
                ...
                Logger.info(
                    "train | epoch: {:d}  step: {:d} | loss: {:.4f}".format(
                        self.epoch, self.global_train_step,
                        loss_accum / (step + 1)))
                ...
                if not self.cfg.mode.nni and self.global_rank == 0 and self.cfg.logger.log:
                    ...
                    mlflow.log_metric("train/acc_atom",
                                      acc_atom_accum / (step + 1),
                                      step=self.global_train_step)
                    mlflow.log_metric("train/acc_pharm",
                                      acc_pharm_accum / (step + 1),
                                      step=self.global_train_step)
                    # mlflow.log_metric("train/acc_amino", acc_amino_accum/(step+1), step=self.global_train_step)
                    mlflow.log_metric("train/acc_edge",
                                      acc_edge_accum / (step + 1),
                                      step=self.global_train_step)
    ...
    def eval_epoch(self, split):
        ...
        if self.global_rank == 0:
            ...
            for metric_name, metric_v in metrics.items():
                if isinstance(
                        metric_v,
                    (float, np.float16, np.float32, np.float64, int, np.int16, np.int32, np.int64)):
                    metric_v = round(metric_v, 5)
                elif isinstance(metric_v, str):
                    metric_v = "\n" + metric_v
                Logger.info("{} |  step: {} | {}: {}".format(
                    split, step, metric_name, metric_v))

            if not self.cfg.mode.nni and self.global_rank == 0 and self.cfg.logger.log:
                for metric_name, metric_v in metrics.items():
                    if isinstance(metric_v, (float, np.float16, np.float32,
                                             np.float64, int, np.int)):
                        mlflow.log_metric("{}/{}".format(split, metric_name),
                                          metric_v,
                                          step=step)
                    elif isinstance(metric_v, str):
                        mlflow.log_text(metric_v,
                                        "{}/{}_report.txt".format(split, step))
        ...
        if split == "valid":
            ...
            if metrics["{}_{}".format(
                    split, self.default_metric)] >= self.best_metric:
                self.best_metric = metrics["{}_{}".format(
                    split, self.default_metric)]

                self.best_model_path = Path("model_step_{}_{}_{}".format(
                    self.global_valid_eval_step, self.default_metric,
                ...
                if self.global_rank == 0 and self.cfg.logger.log:
                    mlflow.pytorch.save_model(
                        pytorch_model=(self.model.module if is_parallel(
                            self.model) else self.model),
                        path=os.path.join(self.output_dir,
                                          self.best_model_path, 'model'),
                        code_paths=[
                            os.path.join(self.root_level_dir, "model"),
                            os.path.join(self.root_level_dir, "utils")
                        ])
                    mlflow.pytorch.save_model(
                        pytorch_model=(self.linear_pred_atoms.module
                                       if is_parallel(self.linear_pred_atoms)
                                       else self.linear_pred_atoms),
                        path=os.path.join(self.output_dir,
                    ...
                    mlflow.pytorch.save_model(
                        pytorch_model=(self.linear_pred_pharms.module
                                       if is_parallel(self.linear_pred_pharms)
                                       else self.linear_pred_pharms),
                        path=os.path.join(self.output_dir,
                                          self.best_model_path,
                                          'linear_pred_pharms'),
                        code_paths=[
                    ...
                    #                 os.path.join(self.root_level_dir, "utils")])
                    mlflow.pytorch.save_model(
                        pytorch_model=(self.linear_pred_bonds.module
                                       if is_parallel(self.linear_pred_bonds)
                                       else self.linear_pred_bonds),
                        path=os.path.join(self.output_dir,
                                          self.best_model_path,
                                          'linear_pred_bonds'),
                        code_paths=[
    ...
    def evaluate(self, split):
        ...
        with torch.no_grad():
            for step, batch in enumerate(
                    tqdm(self.dataloaders[split],
                         desc="Eval_{}".format(split))):
                batch = batch.to(self.device)
                atom_rep, pharm_rep = self.model(
                    batch
                )  # pred_atom = self.linear_pred_atoms(atom_rep[batch.masked_atom_indices])
                ...
                if self.cfg.train.mask_edge:
                    ...
                    acc_edge = torch.tensor(
                        compute_accuracy(pred_edge,
                                         batch.mask_edge_label[:, 0])).to(
                                             self.device)
                    acc_edge_accum += acc_edge
            ...
            metrics = {
                '{}_loss'.format(split):
                loss_accum.item() / (step + 1),
                '{}_acc_atom'.format(split):
                acc_atom_accum.item() / (step + 1),
                '{}_acc_pharm'.format(split):
                acc_pharm_accum.item() / (step + 1),
                # '{}_acc_amino'.format(split): acc_amino_accum.item()/(step+1),
                '{}_acc_edge'.format(split):
                acc_edge_accum.item() / (step + 1)
            }
...
class Contextpred_Trainer(object):
    ...
    def __init__(self, cfg, global_rank, world_size, model_substruct,
                 model_context, dataloaders, criterion, optimizer_substruct,
                 optimizer_context, device, output_dir):
        self.cfg = cfg
        self.global_rank = global_rank
        self.world_size = world_size
    ...
    def train_epoch(self):
        ...
        for step, batch in enumerate(self.dataloaders['train']):
            ...
            substruct_rep = self.model_substruct(
            ...
            overlapped_atom_rep = self.model_context(
            ...
            if self.cfg.train.mode == "cbow":
                ...
                context_rep = self.pool_func(
                    overlapped_atom_rep,
                    batch.batch_overlapped_context,
                    mode=self.cfg.train.context_pooling)
                # negative contexts are obtained by shifting the indicies of context embeddings
                neg_context_rep = torch.cat([
                    context_rep[self.cycle_index(len(context_rep), i + 1)]
                ...
                pred_neg = torch.sum(substruct_rep.repeat(
                    (self.cfg.train.neg_samples, 1)) * neg_context_rep,
                                     dim=1)
                ...
                for i in range(self.cfg.train.neg_samples):
                    shifted_substruct_rep = substruct_rep[self.cycle_index(
                ...
                pred_neg = torch.sum(shifted_expanded_substruct_rep *
                                     overlapped_atom_rep.repeat(
                                         (self.cfg.train.neg_samples, 1)),
                                     dim=1)
            ...
            balanced_loss_accum += float(loss_pos.detach().cpu().item() +
                                         loss_neg.detach().cpu().item())
            acc_accum += 0.5 * (
                float(torch.sum(pred_pos > 0).detach().cpu().item()) /
                len(pred_pos) +
                float(torch.sum(pred_neg < 0).detach().cpu().item()) /
                len(pred_neg))
            ...
            if (self.global_train_step +
                ...
                Logger.info(
                    "train | epoch: {:d}  step: {:d} | loss: {:.4f}".format(
                        self.epoch, self.global_train_step,
                        balanced_loss_accum / (step + 1)))

                if not self.cfg.mode.nni and self.global_rank == 0 and self.cfg.logger.log:
                    mlflow.log_metric("train/balanced_loss",
                                      balanced_loss_accum / (step + 1),
                                      step=self.global_train_step)
                    mlflow.log_metric("train/accuracy",
                                      acc_accum / (step + 1),
                                      step=self.global_train_step)
    ...
    def eval_epoch(self, split):
        ...
        if split == "valid":
            ...
            if metrics["{}_{}".format(
                ...
                if self.global_rank == 0 and self.cfg.logger.log:
                    mlflow.pytorch.save_model(
                        pytorch_model=(self.model_substruct.module
                                       if is_parallel(self.model_substruct)
                                       else self.model_substruct),
                        path=os.path.join(self.output_dir,
                                          self.best_model_path,
                                          'model_substruct'),
                        code_paths=[
                    ...
                    mlflow.pytorch.save_model(
                        pytorch_model=(self.model_context.module
                                       if is_parallel(self.model_context) else
                                       self.model_context),
                        path=os.path.join(self.output_dir,
                                          self.best_model_path,
                                          'model_context'),
    ...
    def evaluate(self, split):
        ...
        with torch.no_grad():
            for step, batch in enumerate(
                ...
                balanced_loss_accum += torch.tensor(
                    loss_pos.detach().cpu().item() +
                    loss_neg.detach().cpu().item()).to(self.device)
                acc_accum += 0.5 * (torch.tensor(
                    torch.sum(pred_pos > 0).detach().cpu().item()).to(
                        self.device) / len(pred_pos) + torch.tensor(
                            torch.sum(pred_neg < 0).detach().cpu().item()).to(
                                self.device) / len(pred_neg))
        ...
        metrics = {
            '{}_balanced_loss'.format(split):
            balanced_loss_accum.item() / (step + 1),
            '{}_accuracy'.format(split):
            acc_accum.item() / (step + 1)
        }
