...
def main(cfg: DictConfig):
    ...
    if global_rank == 0:
        print("num layer: {} mask rate: {:.2f} mask edge: {:.2f}".format(
            cfg.train.num_layer, cfg.train.mask_rate, cfg.train.mask_edge))
    ...
    if cfg.train.model == 'PharmHGT':
        model = PharmHGT(cfg.train.hid_dim, cfg.train.act, cfg.train.num_layer,
                         cfg.train.atom_dim, cfg.train.bond_dim,
                         cfg.train.pharm_dim, cfg.train.reac_dim).to(device)
    ...
    optimizer_list = [
        ...
        optimizer_linear_pred_pharms, optimizer_linear_pred_bonds
    ]
